...
const RIGHT_OFFSET = 473 // x offset for Social Worker copy
...
const FIELDS = {
  serial_number: { x: 402, y: toY(60), size: 7 },
  region: { x: 124, y: toY(88), size: 7 },
  district: { x: 353, y: toY(88), size: 7 },
  province: { x: 140, y: toY(98), size: 7 },
  barangay: { x: 353, y: toY(98), size: 7 },
  city_municipality: { x: 140, y: toY(108), size: 7 },
  evacuation_center: { x: 353, y: toY(108), size: 7 },
  ...
  mothers_maiden_name: { x: 350, y: toY(143), size: 7 },
  ...
  contact_primary: { x: 318, y: toY(225), size: 7 },
  contact_alternate: { x: 390, y: toY(225), size: 7 },
...
const FM_ROWS = [330, 340, 356, 366, 376, 387, 397, 407]
  .map(y => toY(y - 8))
...
async function fillCard(
  ...
  f(fields.birthdate, formatDate(head?.birthdate ?? ''))
  ...
  members.slice(0, 8).forEach((member: FamilyMember, i: number) => {
    ...
    drawText(page, formatDate(member.birthdate ?? ''), FM_COLS.birthdate + xOffset, rowY, rowSize, font)
  ...
  f(fields.date_registered, formatDate(data.date_registered ?? ''))
}
// ROUTE HANDLER
export async function GET(
  ...
  try {  // ← add this back
    ...
    const [regionRes, provinceRes, munRes, barangayRes] = await Promise.all([
      ...
      supabaseAdmin.from('psgc_provinces').select('name').eq('code', card.psgc_code?.substring(0, 5).padEnd(10, '0')).single(),
    ...
    await fillCard(page1, enrichedCard, 0,            font, fields as typeof FIELDS)
    await fillCard(page1, enrichedCard, RIGHT_OFFSET, font, fields as typeof FIELDS)
    ...
    assistanceRecords?.forEach((rec, i) => {
      ...
      both(ASSISTANCE_COLS.date, formatDate(rec.assistance_date ?? ''))
    ...
    return new NextResponse(buffer, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="FACED-${card.serial_number}-${card.family_heads.last_name}.pdf"`,
      },
    })
